...
-----------------------------
By far the easiest way to configure plumetrack is using the plumetrack-config program. This is an interactive, graphical application for creating configuration files. To launch the configuration program run the following command::

    plumetrack-config

You can pass the filename of an existing configuration file to edit on the commandline, e.g.::
...
The plumetrack-config program has two main components. The first is the editor window (which opens when the program is first run) allows all of the configuration settings for the program to be set. The second is the configuration test window, which shows a plot of the motion field calculated using the current settings in the editor window. The configuration can be trialled on different images and the effects of changing different parameters can be viewed in realtime. To open the test window click on the Test button at the bottom of the editor window.
